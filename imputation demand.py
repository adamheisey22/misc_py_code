...
def impute_demand(df, forecast_column, demand_range_table, exception_list):
    for ba in df['BA'].unique():
        ba_mask = df['BA'] == ba

        # Rule 1: Use demand forecast value if valid and not in the exception list
        forecast_mask = ~pd.isna(df[forecast_column]) | df['BA'].isin(exception_list)
        df.loc[ba_mask & forecast_mask & df['Demand'].isna(), 'Demand'] = df[forecast_column]
        ...
        # Rule 1: Use demand forecast value if valid and not in the exception list
        df.loc[ba_mask & range_mask & forecast_mask, 'Demand'] = df[forecast_column]
...
# Example usage:
# df = impute_demand(df, 'Forecast', demand_range_table, exception_list)
