            ...
            if filepath:
                try:
                    with open(filepath, 'w') as f:
                        toml.dump(config, f)
                    messagebox.showinfo(
                        'Success', f"Config file '{Path(filepath).name}' saved successfully!"
                    )
                    edit_window.destroy()
                except Exception as e:
...
def on_run_button_click():
    ...
    if mode:
        ...
        try:
            ...
            messagebox.showinfo(
                'Success',
                f"{mode.capitalize()} mode has finished running. See results in 'output' folder",
            )
        except Exception as e:
...
def setup_argparse():
    parser = argparse.ArgumentParser(
        description='Run the model in different modes with a specified config.'
    )
    parser.add_argument(
        '--config', help='Specify the path to the config file (TOML or CSV).', required=False
    )
